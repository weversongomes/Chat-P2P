  ...
  return function onmessage(event) {
    if (typeof event.data === 'string') {
	  ...
	  if (buf.byteLength < 10000) {
		  console.log('Expecting a message');
		  console.log(event.data);
		  //alert(event.data);
		  renderMessage(event.data);
	  } else {
		  console.log('Expecting a photo');
	  }
      return;
    }
...
function renderPhoto(data) {
  ...
  trail.insertBefore(message, trail.lastChild);
 ...
 */
function renderMessage(receivedMessage) {
	var message = document.createElement('p');
	message.innerHTML = receivedMessage;
	trail.insertBefore(message, trail.lastChild);
}

function renderMyMessage(receivedMessage) {
	var message = document.createElement('p');
	message.innerHTML = receivedMessage;
